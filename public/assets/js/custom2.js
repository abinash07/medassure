...
$('#patientAppointmentForm').submit(function(e)
{
    ...
    city = city.trim();
	var citydata = cities.filter(function (citydata) { return citydata.city == city });
...
$('.online-consult-button').attr('disabled',true);
	...
	$.post(base_url+'/captureuser',$('#patientAppointmentForm').serialize(),function(data){
        ...
        window.location.replace(window.location.href+'&form_step_1=done');
...
function autocomplete(inp, arr) {
  ...
  inp.addEventListener("input", function(e) {
      ...
      for (i = 0; i < arr.length; i++) {
        ...
        if (arr[i]['city'].substr(0, val.length).toUpperCase() == val.toUpperCase().trim()) {
